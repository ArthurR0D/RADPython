...
class SistemaNotas:
    ...
    def definir_estilos_ttk(self):
        ...
        """
        self.style.theme_use('clam')  # 'clam' ou 'alt' geralmente são bons para customização

        # --- Paleta de Cores "Escolares" (Paleta 1: Azul e Verde Suaves) ---
        cor_fundo_principal = "#E0F2F7"  # Azul bebê bem claro
        cor_fundo_secundaria = "white"  # Branco para frames internos e abas selecionadas
        cor_fundo_terciaria = "#F8FCFF"  # Quase branco para contraste leve
        cor_texto_primaria = "#2C3E50"  # Azul petróleo escuro para textos
        cor_texto_secundaria = "#4A4A4A"  # Cinza escuro
        cor_borda = "#B0D8E4"  # Azul suave para bordas
        cor_destaque = "#81C784"  # Verde pastel para cabeçalhos/seleções
        cor_botao_padrao = "#64B5F6"  # Azul céu para botões
        cor_botao_ativo = "#42A5F5"  # Azul mais forte ao passar o mouse
        cor_aprovado = "#4CAF50"  # Verde para aprovação
        cor_reprovado = "#EF5350"  # Vermelho para reprovação
        ...
        self.style.map("TNotebook.Tab",
                       ...
                       bordercolor=[("selected", cor_borda), ("!selected", cor_fundo_principal)])  # Bordas das abas
        ...
        # --- Treeview (Tabelas) ---
        # Configure o estilo base do Treeview. As tags 'oddrow' e 'evenrow' serão configuradas
        # diretamente no objeto Treeview na função self.criar_interface
    ...
    def criar_interface(self):
        ...
        # Essas configurações devem ser feitas APÓS o Treeview ser criado.
        self.tree.tag_configure("oddrow", background="white")
        self.tree.tag_configure("evenrow", background="#F0F8F9")  # Azul muito claro para linhas pares
        self.tree_final.tag_configure("oddrow", background="white")
        self.tree_final.tag_configure("evenrow", background="#F0F8F9")  # Azul muito claro para linhas pares
    ...
    def configurar_aba_consulta(self):
        ...
        ttk.Label(frame_tipo_consulta, text="Tipo de Visualização:", font=('Helvetica', 10, 'bold')).pack(side='left',
                                                                                                          anchor='w',
                                                                                                          padx=(0,
                                                                                                                15))  # Mais padding
        ...
        ttk.Button(frame_filtros_geral, text="Consultar", command=self.executar_consulta).grid(row=4, column=0,
        ...
        self.btn_excluir = ttk.Button(frame_acoes, text="Excluir Registro de Notas", command=self.excluir_registro,
    ...
    def consultar_situacao_final(self):
        ...
        if conn:
            try:
                ...
                if not resultados:
                    self.tree_final.insert("", "end", values=["Nenhum aluno com 2 semestres completos."],
                                           tags=("oddrow",))
                    return
